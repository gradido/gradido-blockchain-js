...
describe('validate transaction body', () => {
  ...
  it('invalid other group', () => {
    const body = new TransactionBody(createdAt, versionString, CrossGroupType_LOCAL, '<script>')
    expect(() => new InteractionValidate(body).run(ValidateType_SINGLE))
      .toThrow('TransactionValidationInvalidInputException: invalid character, only lowercase english latin letter, numbers and - with other_group: string, expected: ^[a-z0-9-]{3,120}$, actual: <script>')
  })
})